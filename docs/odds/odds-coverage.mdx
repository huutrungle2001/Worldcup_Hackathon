...
<Info>
  For each fixture, inspect the returned `SuperOddsType` and market parameters
  from `/api/odds/snapshot/{fixtureId}` or `/api/odds/updates/...`. The
  competition coverage list tells you which competitions can be covered; the
  odds response tells you which markets are actually available for a specific
  fixture.
</Info>
