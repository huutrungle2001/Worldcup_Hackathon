...
<Warning>
  Use all values from one network only. A devnet subscribe transaction from
  `6pW64gN1s2uqjHkn1unFeEjAwJkPGHoppGvS715wyP2J` must be activated with
  `https://txline-dev.txodds.com`, and a mainnet subscribe transaction from
  `9ExbZjAapQww1vfcisDmrngPinHTEfpjYRWMunJgcKaA` must be activated with
...
<Warning>
  Derive the epoch day from the exact timestamp in the proof response, never
  from `Date.now()`. For scores, use
  `validation.summary.updateStats.minTimestamp`; for fixtures, use
  `validation.snapshot.Ts`; for odds, use `validation.odds.Ts`. Recompute it for
  every proof.
</Warning>
