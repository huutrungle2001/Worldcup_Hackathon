...
export class TxLineClient {
  ...
  constructor() {
    ...
    if (!appConfig.apiToken) {
      throw new Error("X_API_TOKEN is missing from config. Please run subscription and activation first.");
    }
  ...
  public async request<T>(config: AxiosRequestConfig, retries = 3, delayMs = 1000): Promise<T> {
    ...
    try {
      ...
      if (status === 401 && retries > 0) {
        logger.warn(`API request returned 401 Unauthorized. Retrying with a refreshed JWT...`);
        await this.refreshJwt();
        return this.request<T>(config, retries - 1, delayMs);
      }

      if ((status === 429 || (status >= 500 && status < 600)) && retries > 0) {
        logger.warn(`API request failed with status ${status}. Retrying in ${delayMs}ms... (Retries left: ${retries})`);
        await new Promise((resolve) => setTimeout(resolve, delayMs));
        return this.request<T>(config, retries - 1, delayMs * 2);
      }

      if (status === 403) {
        logger.error(`API request returned 403 Forbidden. This indicates a subscription configuration or network mismatch.`, err.response?.data || err.message);
  ...
  public async getScoreProof(fixtureId: number | string, seq: number, statKeys: string): Promise<any> {
